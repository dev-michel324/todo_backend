...
class TodoController {
  ...
  async create(req: any, res: any): Promise<any> {
    ...
    const validation = await todoCreateSchema.validate(todo).catch((err) => {
      return err;
    });
    ...
    if (validation.errors)
      return res
        .status(400)
        .json(new ErrorHandler(todo, validation.errors).handle());
    ...
    try {
      ...
      return res.status(500).json(new ErrorHandler(todo, err.message).handle());
    }
  ...
  async update(req: any, res: any): Promise<any> {
    ...
    const validation = await todoUpdateSchema.validate(todo).catch((err) => {
      return err;
    });
    ...
    if (!todoExists)
      return res
        .status(404)
        .json(new ErrorHandler(todo, ["Todo not found."]).handle());
    ...
    try {
      ...
      return res
        .status(500)
        .json(new ErrorHandler(todo, [err.message]).handle());
    }
  ...
  async show(req: any, res: any): Promise<any> {
    ...
    const validation = await todoBaseSchema.validate(todo).catch((err) => {
      return err;
    });
  ...
  async close(req: any, res: any): Promise<any> {
    const todo: TodoBaseType = {
      id: req.params.id
    };
